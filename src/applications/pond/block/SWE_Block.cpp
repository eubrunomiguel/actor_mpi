...
void SWE_Block::computeMaxTimestep( const float i_dryTol,
  ...
  double l_maximumWaveSpeed = (float) 0;

  // compute the maximum wave speed within the grid
  for(int i=1; i <= nx; i++) {
    for(int j=1; j <= ny; j++) {
      if( h[i][j] > i_dryTol ) {
        double l_momentum = std::max( std::abs( hu[i][j] ),
                                     std::abs( hv[i][j] ) );

        double l_particleVelocity = l_momentum / h[i][j];
        
        // approximate the wave speed
        double l_waveSpeed = l_particleVelocity + std::sqrt( g * h[i][j] );
  ...
  double l_minimumCellLength = std::min( dx, dy );
