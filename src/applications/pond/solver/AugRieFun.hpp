 ...
 **************************************************************************************/
		...
		const real hLLMiddleHeight = std::max((huLeft - huRight + extEinfeldtSpeeds[1] * hRight - extEinfeldtSpeeds[0] * hLeft) / (extEinfeldtSpeeds[1] - extEinfeldtSpeeds[0]), static_cast<real>(0));
 ...
 **************************************************************************************/
		...
		beta[1] = rightHandSide[2] 
		        - eigenValues[0]*eigenValues[0]* beta[0]
		        - eigenValues[2]*eigenValues[2]* beta[2];
...
computeMiddleState (
	...
	real& o_hMiddle,
	real o_middleStateSpeeds[2]) const
{
 ...
 **************************************************************************************/
	...
	if (0 <= static_cast<real>(2) * (std::sqrt(g * hMin) - std::sqrt(g * hMax)) + uDif) {
		...
		o_hMiddle = std::max(static_cast<real>(0), i_uLeft - i_uRight + static_cast<real>(2) * (l_sqrt_g_hLeft + l_sqrt_g_hRight));
		...
		for (unsigned int i = 0; i < i_maxNumberOfNewtonIterations; i++) {
			...
			if (std::abs(phi) < newtonTol) break;

			real derivativePhi = l_sqrtTermH[0] + l_sqrtTermH[1] - static_cast<real>(0.25) * g * 
					( (o_hMiddle - i_hLeft) / (l_sqrtTermH[0] * o_hMiddle * o_hMiddle) +
					  (o_hMiddle - i_hRight) / (l_sqrtTermH[1] * o_hMiddle * o_hMiddle)
					);
