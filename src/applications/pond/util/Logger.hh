...
//private:
  ...
  public:
    ...
    Logger( const int i_processRank = 0,
            const std::string i_programName = "Pond",
            const std::string i_welcomeMessage = "Welcome to",
            const std::string i_copyRights =  "\n\nPond Copyright (C) 2012-2018\n"
                                              "  Technical University of Munich\n"
                                              "  Department of Informatics\n"
                                              "  Chair of Scientific Computing\n"
                                              "\n"
                                              "Pond comes with ABSOLUTELY NO WARRANTY.\n"
                                              "Pond is free software, and you are welcome to redistribute it\n"
        ...
        nullOut.open("/dev/null", std::ofstream::out | std::ofstream::app);
    }
     ...
     */
    std::ostream& cout(bool allRanks = true) {
      if (allRanks || processRank == 0) {
        return timeCout() << indentation <<  "process " << processRank << " - ";
      } else {
        return nullOut;
      }
    }
     ...
     */
    void printString(const std::string i_string, bool printTime = true) {
        if (processRank == 0 && printTime) {
            timeCout() << indentation << i_string << std::endl;
     ...
     */
    void initWallClockTime(double i_wallClockTime) {
