...
//    : io::Writer(i_baseName + ".(" + std::to_string(static_cast<int>(i_originX / (i_dX * i_nX))) + "-" + std::to_string(static_cast<int>(i_originY / (i_dY * i_nY))) + ").nc", 
      : io::Writer((i_baseName + ".(" + std::to_string(sa.minX) + "-" + std::to_string(sa.maxX) + ":" + std::to_string(sa.minY) + "-" + std::to_string(sa.maxY) + ").nc"),
             i_b, 
             i_boundarySize, 
             i_nX, 
             i_nY),
      ...
      flush(i_flush)
{
  ...
  //check if the netCDF-file creation constructor succeeded.
	if (status != NC_NOERR) {
        std::cerr << "Unable to open file: " << fileName.c_str() << " - Cause: " <<  nc_strerror(status) << std::endl;
		assert(false);
		return;
	}
	...
	ncPutAttText(NC_GLOBAL, "comment", "Pond is free software and licensed under the GNU General Public License. Remark: In general this does not hold for the used input data.");
...
void io::NetCdfWriter::writeTimeStep(const Float2D &i_h, const Float2D &i_hu, const Float2D &i_hv, float time) {
    if (config::isGasnetSequentialBackend) {
        writeTimeStepInt(i_h, i_hu, i_hv, time);
    } else {
 ...
 */
inline void io::NetCdfWriter::writeTimeStepInt( const Float2D &i_h,
                                      const Float2D &i_hu,
                                      const Float2D &i_hv,
                                      float i_time) {
	if (timeStep == 0)
